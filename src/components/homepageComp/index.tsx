...
const HomePageComponent = () => {
  ...
  const address = [
    {line:"10/7 Ruk Nivas,"},
    {line:"Raja Street, Devaraj Nagar, Saligramam,"},
    {line:"Chennai 600093,"},
  ]
  ...
  return (
    <div className='w-full h-full flex flex-col gap-5'>
      ...
      <div className="w-full h-full flex gap-3 justify-center lg:justify-start items-center lg:items-start flex-col lg:flex-row">       

        <aside className="w-2/3 h-full flex flex-col p-3 locationBox">
        ...
        <aside className="w-1/3 h-full flex flex-row gap-3 justify-start items-start">
          <img src={Location} alt="Location" width={25}/>
          
          
          {address.map((item, index) => (
            <aside key={index}>
              <p className='italic font-semibold text-xl '> {item.line} </p>
            </aside>            
          ))}
      ...
      {/* <div className="w-full h-full flex gap-3 justify-center lg:justify-start items-center lg:items-start flex-col lg:flex-row" style={{ border: "2px solid red" }}>
        <h1>Homie's Location</h1>
        <iframe
            width="100%"
            height="300"
            style={{ border: 0 }}
      ...
      <div className='w-full h-full' style={{ border: "2px solid red"}}>
...
<iframe
      ...
      <Box className='w-full h-1/4' sx={{border:"2px solid red", display:{xl:"flex", lg:"flex", md:"none", sm:"none", xs:"none"}}}>
        
      </Box>

      <Box className='w-full flex items-start justify-center gap-5 p-3' sx={{border:"2px solid red" ,flexDirection:{xl:"row", lg:"row", md:"row", sm:"column", xs:"column"}}}>
        <Box className='lg:w-2/5 md:w-full sm:w-full xs:w-full h-full flex lg:justify-start lg:items-start' sx={{
          border:"2px solid blue"
        }}>
          <img src={Homies} alt="Homies" width={500}/>
        </Box>

        <aside className='lg:w-3/4 md:w-full h-full flex flex-col justify-center lg:items-start sm:items-center gap-3'>
          <h1 className='text-3xl font-extrabold lg:text-6xl xl:text-6xl'>de Power House</h1>
          <p className='text-wrap text-justify italic font-semibold text-md lg:text-xl xl:text-xl'>
            We roll as five, but we move as one. Three Chartered Accountants who balance numbers with precision,
            one Cost Management Accountant who finds value in every detail, and one techie who codes solutions into reality.
      ...
      <div className='w-full h-full flex justify-center items-center' style={{border:"2px solid red"}}>
